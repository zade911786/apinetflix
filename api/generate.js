export default async function handler(req, res) {
  ...
  function extractCookies(text) {
    ...
    try {
      ...
      if (Array.isArray(json) && json.length && json[0]?.name) {
        json.forEach(c => {
          if (keys.includes(c.name)) cookies[c.name] = c.value;
        });
      } else if (json && typeof json === 'object') {
        keys.forEach(k => {
          if (json[k] !== undefined) cookies[k] = json[k];
        });
      }
    } catch (e) {}
    ...
    for (let segment of segments) {
      ...
      let value = segment.substring(eqPos + 1).trim();
      value = value.replace(/;+$/, '');
  ...
  if (!cookie_dict.NetflixId || !cookie_dict.NetflixId.trim()) {
  ...
  try {
    ...
    if (!apiResponse.ok) {
      return res.status(status).json({ error: `Netflix API returned HTTP ${status}` });
    }
